...
where $r_{f, t}$ is the fund's return and $\overline{r_f}$ represents the mean of returns over $T$ periods. Then scaling via  $m$, the return frequency within a year, yields the standard deviation of yearly returns. For illustrative purposes, the calculation using weekly returns would result in $m = 52$, as there are 52 months in a year and $T = 5 * 52 = 260$ the number of months in 5 years. To obtain the SRRI which is measured on a scale from 1 to seven the regulating autorithy provides a table.
\begin{center}
\begin{tabular}{l|l|l}
	\hline
	SRRI & $\sigma_f$ greater or equal to & $\sigma_f$ less than\\
	\hline
	1 & 0\% & 0.5\%\\
	\hline
	2 & 0.5\% & 2\%\\
